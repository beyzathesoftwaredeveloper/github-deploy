...
aws s3 mb s3://mern-terraform-state --region eu-central-1
...
```bash
# kubeconfig güncelle
aws eks update-kubeconfig --region eu-central-1 --name mern-devops-case-eks-cluster

# Image URL'lerini gerçek değerlerle replace et
sed -i "s/YOUR_AWS_ACCOUNT_ID/686519987622/g" kubernetes/mern/*.yaml
sed -i "s/eu-central-1/$AWS_REGION/g" kubernetes/mern/*.yaml kubernetes/python-etl/*.yaml
